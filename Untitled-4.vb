...
' --------------------------------------------------------------------------------------
' [8b] ADHERENCE CHARTS + WEEKLY/MONTHLY DASHBOARD REPORTS
' --------------------------------------------------------------------------------------
Public Sub AddAdherenceSummaryChart()
    ...
    Set dashWS = ThisWorkbook.Sheets("PerformanceDashboard")
    If dashWS Is Nothing Then Exit Sub
    ...
    ' Prepare summary by state type for today
    Dim dict As Object: Set dict = CreateObject("Scripting.Dictionary")
    Dim r As Long, state As String, duration As Double
    For r = 2 To lastRow
        If ws.Cells(r, 1).Value = Date Then
            state = ws.Cells(r, 2).Value
            duration = ws.Cells(r, 5).Value
            If Not dict.exists(state) Then
                dict.Add state, duration
            Else
                dict(state) = dict(state) + duration
            End If
        End If
    Next r
    ...
    ' Write summary into dashboard
    Dim i As Long: i = dashWS.Cells(dashWS.Rows.Count, 1).End(xlUp).Row + 3
    dashWS.Cells(i, 1).Value = "Adherence Summary"
    dashWS.Range("A" & i & ":B" & i).Font.Bold = True
    dashWS.Cells(i + 1, 1).Value = "State"
    dashWS.Cells(i + 1, 2).Value = "Total Duration (hrs)"
    Dim rowOffset As Long: rowOffset = i + 2
    For Each state In dict.Keys
        dashWS.Cells(rowOffset, 1).Value = state
        dashWS.Cells(rowOffset, 2).Value = Round(dict(state) * 24, 2) ' convert from Excel time to hours
        rowOffset = rowOffset + 1
    Next state

    ' Create chart
    Set chartObj = dashWS.ChartObjects.Add(Left:=300, Width:=500, Top:=300, Height:=250)
    With chartObj.Chart
        .SetSourceData dashWS.Range("A" & (i + 1) & ":B" & (rowOffset - 1))
        .ChartType = xlColumnClustered
        .HasTitle = True
        .ChartTitle.Text = "Today's Adherence (Total Hours by Type)"
    End With


' --------------------------------------------------------------------------------------
' [8c] WEEKLY AND MONTHLY ADHERENCE CHARTS
' --------------------------------------------------------------------------------------
Public Sub AddWeeklyMonthlyAdherenceCharts()
    Dim ws As Worksheet, dashWS As Worksheet, dictW As Object, dictM As Object
    On Error Resume Next
    Set ws = ThisWorkbook.Sheets("Adherence_Log")
    Set dashWS = ThisWorkbook.Sheets("PerformanceDashboard")
    If ws Is Nothing Or dashWS Is Nothing Then Exit Sub
    On Error GoTo 0

    Set dictW = CreateObject("Scripting.Dictionary")
    Set dictM = CreateObject("Scripting.Dictionary")

    Dim r As Long, state As String, duration As Double, logDate As Date
    Dim todayW As Date: todayW = Date - Weekday(Date, vbMonday) + 1 ' this week Monday
    Dim startM As Date: startM = DateSerial(Year(Date), Month(Date), 1) ' 1st of month

    For r = 2 To ws.Cells(ws.Rows.Count, 1).End(xlUp).Row
        logDate = ws.Cells(r, 1).Value
        state = ws.Cells(r, 2).Value
        duration = ws.Cells(r, 5).Value
        If logDate >= todayW Then
            If Not dictW.exists(state) Then dictW(state) = 0
            dictW(state) = dictW(state) + duration
        End If
        If logDate >= startM Then
            If Not dictM.exists(state) Then dictM(state) = 0
            dictM(state) = dictM(state) + duration
        End If
    Next r

    Dim iW As Long: iW = dashWS.Cells(dashWS.Rows.Count, 1).End(xlUp).Row + 3
    dashWS.Cells(iW, 1).Value = "Weekly Adherence Summary"
    dashWS.Range("A" & iW & ":B" & iW).Font.Bold = True
    dashWS.Cells(iW + 1, 1).Value = "State"
    dashWS.Cells(iW + 1, 2).Value = "Total Hours"

    Dim rowW As Long: rowW = iW + 2
    For Each state In dictW.Keys
        dashWS.Cells(rowW, 1).Value = state
        dashWS.Cells(rowW, 2).Value = Round(dictW(state) * 24, 2)
        rowW = rowW + 1
    Next

    Dim chartW As ChartObject
    Set chartW = dashWS.ChartObjects.Add(Left:=20, Width:=400, Top:=rowW * 15, Height:=200)
    With chartW.Chart
        .SetSourceData dashWS.Range("A" & (iW + 1) & ":B" & (rowW - 1))
        .ChartType = xlColumnClustered
        .HasTitle = True
        .ChartTitle.Text = "Weekly Adherence Summary (Hrs)"
    End With

    Dim iM As Long: iM = rowW + 3
    dashWS.Cells(iM, 1).Value = "Monthly Adherence Summary"
    dashWS.Range("A" & iM & ":B" & iM).Font.Bold = True
    dashWS.Cells(iM + 1, 1).Value = "State"
    dashWS.Cells(iM + 1, 2).Value = "Total Hours"

    Dim rowM As Long: rowM = iM + 2
    For Each state In dictM.Keys
        dashWS.Cells(rowM, 1).Value = state
        dashWS.Cells(rowM, 2).Value = Round(dictM(state) * 24, 2)
        rowM = rowM + 1
    Next

    Dim chartM As ChartObject
    Set chartM = dashWS.ChartObjects.Add(Left:=450, Width:=400, Top:=rowM * 15, Height:=200)
    With chartM.Chart
        .SetSourceData dashWS.Range("A" & (iM + 1) & ":B" & (rowM - 1))
        .ChartType = xlColumnClustered
        .HasTitle = True
        .ChartTitle.Text = "Monthly Adherence Summary (Hrs)"
    End With
End Sub

AddWeeklyMonthlyAdherenceCharts
