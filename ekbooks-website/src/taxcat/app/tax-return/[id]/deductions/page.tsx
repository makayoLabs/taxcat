...
export default function DeductionsPage({ params }: { params: { id: string } }): void {
  ...
  useEffect(() => {
    const total = deductions.reduce((sum, ___deduction) => sum + (deduction.amount || 0), 0);
    setTotalDeductions(total);
  }, [deductions]);

  const onSubmit = async (___data: DeductionsFormData) => {
    try {
          ...
          {!useStandardDeduction && (
            <div className="bg-white p-6 rounded-lg shadow">
              ...
              <div className="space-y-6">
                {deductionFields.map((field, ___index) => (
                  <div key={field.id} className="border p-4 rounded-md relative">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div>
                        ...
                        >
                          {deductionCategories.map((___category) => (
