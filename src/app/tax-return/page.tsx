          ...
          {loading ? (
            ...
            <div className="divide-y divide-gray-200">
              {taxReturns.map((_taxReturn) => (
                <div key={_taxReturn.id} className="p-6 hover:bg-gray-50 transition-colors">
                  <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center">
                        ...
                        <div className="ml-4">
                          <h3 className="text-lg font-medium text-gray-900">
                            {_taxReturn.year} Tax Return
                          </h3>
                          <p className="text-sm text-gray-600">
                            Status: <span className={`font-medium ${
                              _taxReturn.status === 'COMPLETED' ? 'text-green-600' :
                              _taxReturn.status === 'IN_PROGRESS' ? 'text-blue-600' :
                              _taxReturn.status === 'DRAFT' ? 'text-gray-600' :
                              'text-red-600'
                            }`}>
                              {_taxReturn.status.replace('_', ' ')}
                            </span>
                          </p>
                          <p className="text-sm text-gray-600">
                            Filing Status: {_taxReturn.filingStatus.replace('_', ' ')}
                          </p>
                        </div>
                      </div>
                    </div>
                    <div className="flex items-center space-x-4">
                      <div className="text-right">
                        ...
                        <p className="text-lg font-medium text-gray-900">
                          ${_taxReturn.totalIncome.toLocaleString()}
                        </p>
                        {_taxReturn.refundAmount > 0 ? (
                          <p className="text-sm text-green-600">
                            Refund: ${_taxReturn.refundAmount.toLocaleString()}
                          </p>
                        ) : (
                          <p className="text-sm text-red-600">
                            Balance: ${Math.abs(_taxReturn.balanceOwing).toLocaleString()}
                          </p>
                        )}
                      </div>
                      <Link
                        href={`/tax-return/new/${_taxReturn.id}`}
