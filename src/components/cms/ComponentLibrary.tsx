...
const ComponentLibrary = ({ onClose }: ComponentLibraryProps) => {
        ...
        <div className="grid gap-4">
          {AVAILABLE_BLOCKS.map((___block) => (
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              onClick={() => handleAddBlock(___block.type)}
            ...
            >
              <div className="flex-shrink-0 text-taxcat-blue">{___block.icon}</div>
              <div className="ml-4">
                <h3 className="text-sm font-medium text-gray-900">{___block.label}</h3>
                <p className="mt-1 text-sm text-gray-500">{___block.description}</p>
              </div>
            </motion.button>
          ))}
        </div>
