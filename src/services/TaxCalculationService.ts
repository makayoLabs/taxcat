...
export class TaxCalculationService {
  ...
  private calculateTotalIncome(income: Income): Decimal {
    return this.sumEmploymentIncome(income)
      ...
      .plus(Object.values(income.other).reduce((sum, val) => sum.plus(val), new Decimal(0)));
  }
  ...
  private calculateContributions(
    ...
    return {
      cpp: totalEmploymentIncome.times(new Decimal(0.0595)).toDecimalPlaces(2),
      ei: totalEmploymentIncome.times(new Decimal(0.0163)).toDecimalPlaces(2),
      qpp: new Decimal(0),
    };
  }
  ...
  private sumBusinessIncome(income: Income): Decimal {
    return income.business.reduce(
      (sum, bus) => {
        const expenses = Object.values(bus.expenses).reduce((expSum, exp) => expSum.plus(exp), new Decimal(0));
